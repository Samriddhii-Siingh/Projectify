import React from "react";
import "./Project_Mentor.css";
function Project() {
  return (
    <div className="project-body-container">

      <div class="project-card">
        <h3 class="project-card-title">Project 1 Name</h3>
        <span class="project-card-id">Project Id</span>
        <div class="project-card-mentor">Mentor Name</div>
        <div class="project-card-domain">Domain Name</div>
        <hr />
        <div class="project-card-desc">
          This is project 1 and it will be displayed like this as much as I want.
        </div>
      </div>


      <div className="project-card">
        <h3 className="project-card-title">Project 1 Name</h3>
        <span className="project-card-id">Project Id</span>
        <div className="project-card-mentor">Mentor Name</div>
        <div className="project-card-domain">Domain Name</div>
        <hr />
        <div className="project-card-desc">
          this is a project 1 and it will be displayed like this as must as i
          want
        </div>
      </div>


      <div className="project-card">
        <h3 className="project-card-title">Project 1 Name</h3>
        <span className="project-card-id">Project Id</span>
        <div className="project-card-mentor">Mentor Name</div>
        <div className="project-card-domain">Domain Name</div>
        <hr />
        <div className="project-card-desc">
          this is a project 1 and it will be displayed like this as must as i
          want
        </div>
      </div>

      <div className="project-card">
        <h3 className="project-card-title">Project 1 Name</h3>
        <span className="project-card-id">Project Id</span>
        <div className="project-card-mentor">Mentor Name</div>
        <div className="project-card-domain">Domain Name</div>
        <hr />
        <div className="project-card-desc">
          this is a project 1 and it will be displayed like this as must as i
          want
        </div>
      </div>

      <div className="project-card">
        <h3 className="project-card-title">Project 1 Name</h3>
        <span className="project-card-id">Project Id</span>
        <div className="project-card-mentor">Mentor Name</div>
        <div className="project-card-domain">Domain Name</div>
        <hr />
        <div className="project-card-desc">
          this is a project 1 and it will be displayed like this as must as i
          want
        </div>
      </div>

      <div className="project-card">
        <h3 className="project-card-title">Project 1 Name</h3>
        <span className="project-card-id">Project Id</span>
        <div className="project-card-mentor">Mentor Name</div>
        <div className="project-card-domain">Domain Name</div>
        <hr />
        <div className="project-card-desc">
          this is a project 1 and it will be displayed like this as must as i
          want
        </div>
      </div>

      <div className="project-card">
        <h3 className="project-card-title">Project 1 Name</h3>
        <span className="project-card-id">Project Id</span>
        <div className="project-card-mentor">Mentor Name</div>
        <div className="project-card-domain">Domain Name</div>
        <hr />
        <div className="project-card-desc">
          this is a project 1 and it will be displayed like this as must as i
          want
        </div>
      </div>

      <div className="project-card">
        <h3 className="project-card-title">Project 1 Name</h3>
        <span className="project-card-id">Project Id</span>
        <div className="project-card-mentor">Mentor Name</div>
        <div className="project-card-domain">Domain Name</div>
        <hr />
        <div className="project-card-desc">
          this is a project 1 and it will be displayed like this as must as i
          want
        </div>
      </div>

      <div className="project-card">
        <h3 className="project-card-title">Project 1 Name</h3>
        <span className="project-card-id">Project Id</span>
        <div className="project-card-mentor">Mentor Name</div>
        <div className="project-card-domain">Domain Name</div>
        <hr />
        <div className="project-card-desc">
          this is a project 1 and it will be displayed like this as must as i
          want
        </div>
      </div>

      <div className="project-card">
        <h3 className="project-card-title">Project 1 Name</h3>
        <span className="project-card-id">Project Id</span>
        <div className="project-card-mentor">Mentor Name</div>
        <div className="project-card-domain">Domain Name</div>
        <hr />
        <div className="project-card-desc">
          this is a project 1 and it will be displayed like this as must as i
          want
        </div>
      </div>
      
    </div>
  );
}
export default Project;
